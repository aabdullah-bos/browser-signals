...
def mark_failure(driver, message: str, use_flash: bool = True, frame: Optional[object] = None):
    ...
    _inject_banner(
        driver, f"❌ TEST FAILED: {message}", color="red", frame=frame)
    if use_flash:
...
def mark_warning(driver, message: str, use_flash: bool = False, frame: Optional[object] = None):
    ...
    _inject_banner(
        driver, f"⚠️ TEST WARNING: {message}", color="#d97706", frame=frame)
    if use_flash:
...
def show_banner(
    ...
    *,
    ...
    if full_banner:
        ...
        _inject_banner(driver, text, color=use_color,
                       duration_ms=long_ms, frame=frame)
    else:
        use_color = color or "#facc15"  # default amber
        chip_id = id or "browser-signals-running"
        _inject_chip(driver, text, color=use_color, position=position,
                     persistent=persistent, duration_ms=duration_ms, elem_id=chip_id, frame=frame)
...
def remove_banner(driver, *, id: Optional[str] = None, full_banner: bool = False, frame: Optional[object] = None):
    ...
    target_id = id or (
        "browser-signals-banner" if full_banner else "browser-signals-running")
    script = f"""
  (function() {{
...
def _console(driver, message: str, level: str = "log"):
    ...
    driver.execute_script(
        f"console.{level}('BROWSER_SIGNALS [' + Date.now() + 'ms]: ' + '{safe}');")
